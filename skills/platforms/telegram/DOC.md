...
- **SPA-readiness gate (load-bearing).** The very first thing `run()` does is a `page.waitForFunction` poll for two signals:
  ...
  Without this gate, downstream `findGetGlobal` / `findCallApi` walks fail intermittently because the SPA is still booting (commit `cedf7db` — restored after `32a698a` mistakenly stripped `init()` as "redundant"). For SPA-style sites in general, see `skills/openweb/knowledge/extraction.md` § SPA Hydration Gate.
