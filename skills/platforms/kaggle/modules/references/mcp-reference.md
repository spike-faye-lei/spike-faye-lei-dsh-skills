...
- ✅ `list_competition_pages` — host-authored overview pages (rules, evaluation, data-description, FAQ, prizes, timeline). Universal: works for regular competitions, playgrounds, and hackathons. See [competition-overview.md](../competitions/references/competition-overview.md) for the full reference and patterns. Wrapper script: `modules/competitions/scripts/competition_pages.py`.
...
### Hackathon (newer surface — see `modules/competitions/hackathons/`)
...
Prefer `get_writeup` when a writeup id is available because it has the simplest
argument shape. The `modules/competitions/hackathons/scripts/fetch_writeup.py`
script encodes the fallback chain (`get_writeup` → `get_writeup_by_topic` →
`get_writeup_by_slug`).
